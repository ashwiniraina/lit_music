...
def transform_song_vectors(user_id, method):
    ...
    train_sessions_file = open('../datasets/lastfm-dataset-1K/extracts/training_sessions_'+user_id, 'r')
    train_songs_file = open('../datasets/lastfm-dataset-1K/extracts/train_songs_'+user_id, 'w')
    firstline = True
    for line in train_sessions_file:
        newline = line.strip('\n')
        if newline:
            if firstline == True:
                newline = line.strip('\n')
                ...
                newline = ','+line.strip('\n')
            train_songs_file.write(newline)
    train_sessions_file.close()
    train_songs_file.close()
    ...
    # print (np.array(list(train_songs)))
